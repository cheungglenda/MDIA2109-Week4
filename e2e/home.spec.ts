...
/** importing all tests from playwright */
/** beforeAll, afterAll, all the yellow texts are imported and we're using playwright to run these tests. these yellow texts run before everything else runs and at the end will say, "After all", do this. 
 * it goes in random order, and that is the case for unit testing. 
 * can have hundreds of tests and it will go in random order. 
 * async is for waiting for a result, so it says to not run everything at once.*/


var homePage = 'http://localhost:3000'
/** have to be on localhost:3000 for assignment 1 */
...
test.describe('Head tag area', () => {
    test('Contains the Head tag and its contents', async ({ page }) => {
        ...
        /** indicates that title must contain Example #1 */
...
test.describe("Main content tests", () => {
    ...
    test('Should contain a list to describe CRUD', async ({ page }) => {
        await page.goto(homePage);
        /* can specify order of tags, for instance, like using ul > li */
